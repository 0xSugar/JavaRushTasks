...
public class Solution {
    ...
    private static void encrypt(String input, String output) throws IOException {
        ...
        while (rd.ready()) {
            wr.write(changeToEncrypt((char) rd.read()));
        }
        rd.close();
        wr.close();
    }

    private static void decrypt(String input, String output) throws IOException {
        BufferedReader rd = new BufferedReader(new InputStreamReader(new FileInputStream(input)));
        BufferedWriter wr = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(output)));
        while (rd.ready()) {
            wr.write(changeToDecrypt((char) rd.read()));
        }
        rd.close();
        wr.close();
    }

    private static char changeToDecrypt(char c) {
        if(c == '\u203D') {
            return '\n';
        }
        if(c == '\u24D0') {
    ...
    private static char changeToEncrypt(char c) {
        if(c == 'A') {
            return '\u03B1';
        }
        if(c == 'B') {
